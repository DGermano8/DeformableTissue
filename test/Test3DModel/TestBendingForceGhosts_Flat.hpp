...
public:
     ...
     */
    void TestCubeWithGhosts() throw (Exception)
    {
    	RandomNumberGenerator::Instance()->Reseed(2);

        std::vector<Node<3>*> nodes;

        std::string output_directory = "Test_Flat";
        ...
        double width_space = 0.8;
        double height_space = 0.8*sqrt(0.75);
        ...
        double radius =  0.0;//periodic_width+1.0;q
...
//        cell_population.InitialiseCells();
        ...
        MAKE_PTR(CryptModelInteractionForceWithGhostNodes<3>, spring_force);
        spring_force->SetUseOneWaySprings(false); //turning this on makes the stromal cells act as ghosts..
        spring_force->SetCutOffLength(1.2);
        //                     SetEpithelialStromalCellDependentSprings(ind , Ep-Ep, Str-Str, Ep-Str, apcTwoHitStromalMultiplier);
        spring_force->SetEpithelialStromalCellDependentSprings(true, 1.0,     0.5,     0.5,    1.0);
        spring_force->SetPeriodicDomainWidth(periodic_width);
        spring_force->SetPeriodicDomainDepth(periodic_height);
        spring_force->SetMeinekeSpringStiffness(spring_strength);
        if(include_springs)
        {
        ...
        MAKE_PTR(BendingForce3dHeightWithGhostNodes, bending_force);
        bending_force->SetHeightDependantCurvatureParameter(1.2);
        bending_force->SetBasementMembraneParameter(beta_parameter);
        bending_force->SetExponentParameter(alpha_parameter);
        // bending_force->SetCircularNonZeroTargetCurvatureRegion(true, target_curvature, radius, 20, 20);
        bending_force->SetCircularNonZeroTargetCurvatureRegion(true, target_curvature, radius, centre_x, centre_y);
        bending_force->SetPeriodicDomainWidth(periodic_width);
        bending_force->SetPeriodicDomainDepth(periodic_height);
        if(include_bending)
        {
        ...
        // bending_force->AddForceContribution(cell_population);
