...
public:
    ...
    void TestSimpleCubeBending() throw (Exception)
    {
        ...
        unsigned ghosts_bottom = 2;       // ghosts > depth
        unsigned ghosts_top = 2;       // ghosts > depth

        unsigned depth = 3 + (ghosts_bottom + ghosts_top);        // z
...
//        double periodic_height = (double) height + 0.0;
        ...
        // // Initialise Bottom Ghost cells 
        for (unsigned i=0; i<num_ghosts_bottom; i++)
		{
		...
		for (unsigned i=num_ghosts_bottom; i<num_nodes-(num_epithelial_cells + num_ghosts_top); i++)
		{
        ...
        for (unsigned i=num_nodes-(num_epithelial_cells + num_ghosts_top); i<num_nodes - num_ghosts_top; i++)
        {
            real_node_indices.push_back(i);
        ...
        // // Initialise Top Ghost cells 
        for (unsigned i=num_nodes - num_ghosts_top; i<num_nodes; i++)
		{
            ghost_node_indices.push_back(i);

        }

        MeshBasedCellPopulationWithGhostNodes<3> cell_population(*p_mesh, cells, real_node_indices);
 ...
 */
     ...
     // AnoikisCellKiller3D anoikis(&cell_population);
		...
		simulator.SetEndTime(0.5);							// Firstly run to a steady state (i.e. til the thing is deformed as much as it will be)
