...
public:
    ...
    void TestPeriodicCubeWithGhosts() throw (Exception)
    {
        ...
        std::string output_directory = "RandomToFlat_WithArea_20210721_03";
        ...
        for (unsigned k=0; k<(depth + 1 ); k++) //+1 puts a layer of ghosts on the bottom
        {
            ...
            {    
                for (unsigned i=0; i<width; i++)
                {
                    ...
                    x_coordinate = (double) (i + 0.5*(j%2 + k%2))*width_space       + 0.05*(2.0*RandomNumberGenerator::Instance()->ranf()-1.0);
                    y_coordinate = (double) j*height_space                          + 0.05*(2.0*RandomNumberGenerator::Instance()->ranf()-1.0);
