...
void PeriodicBendingForce3dHeightWithGhostNodes::SetTissueBase(double tissue_base)
{
	mTissueBase = tissue_base;
}

double PeriodicBendingForce3dHeightWithGhostNodes::GetTissueBase()
{
	return mTissueBase;
}
...
void PeriodicBendingForce3dHeightWithGhostNodes::AddForceContribution(AbstractCellPopulation<3>& rCellPopulation)
{
	...
	double tissue_base = GetTissueBase();
         ...
         ++cell_iter)
    {
		...
		// Move this to it's own force, so we can have tests to check this is calculating correctly
		c_vector<double, 3> force_due_to_drift;
		force_due_to_drift[0] = 0.0; force_due_to_drift[1] = 0.0;
		// PRINT_VARIABLE(CryptMiddle);
		// force_due_to_drift[2] = -1.0*(tissue_min_height - tissue_base)/(SimulationTime::Instance()->GetTimeStep());
		force_due_to_drift[2] = -1.0*(average_height - tissue_base)/(SimulationTime::Instance()->GetTimeStep());

		// rCellPopulation.GetNode(cell_i_ext)->AddAppliedForceContribution(force_due_to_drift);
...
void PeriodicBendingForce3dHeightWithGhostNodes::OutputForceParameters(out_stream& rParamsFile)
{
	...
	*rParamsFile <<  "\t\t\t<TissueBase>"<<  mTissueBase << "</TissueBase> \n" ;	
