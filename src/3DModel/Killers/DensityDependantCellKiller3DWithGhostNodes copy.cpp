 ...
 */
std::set<unsigned> DensityDependantCellKiller3DWithGhostNodes::GetNeighbouringNodeIndices(unsigned nodeIndex)
{
 ...
 */
bool DensityDependantCellKiller3DWithGhostNodes::IsCellTooSmall(unsigned nodeIndex)
{
	DomMeshBasedCellPopulationWithGhostNodes<3>* p_tissue = static_cast<DomMeshBasedCellPopulationWithGhostNodes<3>*> (this->mpCellPopulation);
   							...
   							++neighbour_iter)
	{
		...
		if ( (!p_tissue->IsGhostNode(*neighbour_iter))
				...
				&& !(p_cell_n->HasApoptosisBegun()) )
   		{
			...
			average_cell_distance = average_cell_distance + sqrt( pow(x_location - x_location_n,2) + pow(y_location - y_location_n,2) + pow(z_location - z_location_n,2));
    	 ...
    	 ++cell_iter)
	{
		...
		if(!p_tissue->IsGhostNode(node_index))
		{
			...
			if (cell_iter->GetMutationState()->IsType<StromalCellMutationState>()==false)
			{
				...
				if((!cell_iter->IsDead()) && (p_cell->GetAge() > 1) && (this->IsCellTooSmall(node_index)==true) )
				{
					...
					// PRINT_VARIABLE((this->IsCellTooSmall(node_index)));
