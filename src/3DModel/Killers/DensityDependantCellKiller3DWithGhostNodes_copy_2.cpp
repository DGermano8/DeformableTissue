...
bool DensityDependantCellKiller3DWithGhostNodes::IsCellTooSmall(MutableMesh<3,3>* ExtendedMesh, std::map<unsigned, unsigned> ExtendedMeshNodeIndexMap, unsigned nodeIndex)
{
	...
	PRINT_3_VARIABLES(nodeIndex,num_epithelial_neighbours,average_cell_distance);
...
std::vector<c_vector<unsigned, 3> > DensityDependantCellKiller3DWithGhostNodes::GetEpithelialMesh(AbstractCellPopulation<3>& rCellPopulation)
{
